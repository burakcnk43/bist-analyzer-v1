...
logger = logging.getLogger("StockRobustness")

def main():
    logger.info("--- PHASE 5: STOCK-LEVEL ROBUSTNESS AUDIT ---")
    ...
    # Stock Evaluation
    symbols = global_df['symbol_col'].unique()
    results = []

    for sym in symbols:
        stock_test = holdout_df[holdout_df['symbol_col'] == sym]
        stock_test_clean = stock_test[selected_features + [target_cls]].dropna()

        if stock_test_clean.empty: continue

        y_pred = pipeline.predict(stock_test_clean[selected_features])
        acc = accuracy_score(stock_test_clean[target_cls], y_pred)

        results.append({
            "Symbol": sym,
            "N": len(stock_test_clean),
            "Accuracy": acc
        })

    res_df = pd.DataFrame(results).sort_values("Accuracy", ascending=False)
    res_df.to_csv(DATA_REPORTS_DIR / "PHASE5_STOCK_ROBUSTNESS.csv", index=False)

    print("\nStock Robustness Summary (Top 10):")
    print(res_df.head(10).to_markdown(index=False))

    print("\nStock Robustness Summary (Bottom 10):")
    print(res_df.tail(10).to_markdown(index=False))
